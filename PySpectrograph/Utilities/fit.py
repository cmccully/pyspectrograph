...
class curfit:
    ...
    * order - order of the function that is fit
    ...
    def erf_weights(self, coef, x, y, v, weights=None):
        """Error function to be minimized in least-squares fit that includes weights for the points."""
        if weights is None:
    ...
    def sigma(self, x, y):
        ...
        #Calculate the Absolute median deviation instead
        return (((y-self(x))**2).mean())**0.5
        #return np.median(np.abs(y - self(x)))
...
class interfit(curfit):
    ...
    * niter - number of times to iterate
    ...
    def fitfun(self,f,p0,x,y):
        #Initialize the weights to 1's
        weights = np.ones(len(x))
        thiserrfunc = lambda p: weights * (y - f(p,x))
        #for each iteration
        for i in range(4):
            #do a linear least squares fit
            p1,success = optimize.leastsq(thiserrfunc, p0 )
            #calcualte the biweights
            #residuals
            r = y - f(p1,x)
            #calculate the median absolute deviation and normalize it to 50% confidence level (6745 sigma for a gaussian)
            s = np.median(abs(r - np.median(r))) / 0.6745 
            biweight = lambda x: ((abs(x)<1.0)*(1.0 - x**2)**2.0)**0.5
            weights = biweight(r/s)
        return p1
    ...
    def set_mask(self, init=False):
        ...
        diff=self.y-self(self.x)
        if False:
            f = lambda p,x: p[0]+p[1]* x + p[2]*x*x
            p1 = self.fitfun(f,[0.0,0.0,0.0],self.x,diff)
            sigma = np.median(np.abs(diff - f(p1,self.x)))
        else:
