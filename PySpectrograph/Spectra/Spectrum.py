...
def smooth(spec, smoothwidth):
    ...
    kernelwidth = int(smoothwidth/wavespace)
    #make sure the kernel width is odd
    if kernelwidth % 2 == 0:
        kernelwidth += 1
    kernel = np.ones(kernelwidth)
    #Conserve flux
    kernel /= kernel.sum()
    smoothed = spec.flux.copy()
    smoothed[(kernelwidth/2):-(kernelwidth/2)] = np.convolve(spec.flux, kernel, mode='valid')
    return smoothed
